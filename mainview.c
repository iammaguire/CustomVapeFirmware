...
int ballY = 65;
int initialized = 0;
static int blocks[blocksX][blocksY];
...
__myevic__ void ShowMainView()
{
	if(!initialized) {
		...
		for(int i = 0; i < blocksX; i++) {
			for(int j = 0; j < blocksY; j++) {
				if((i + j) % 2 == 0) blocks[i][j] = 1;
			}
		}
	}

	if(UserInputs == 2 && barX - barSpeed > 0)
		barX -= barSpeed;
	else if(UserInputs == 3 && barX + barWidth + barSpeed < width)
		barX += barSpeed; 
	
	if(ballX + ballSize + ballVX >= width || ballX + ballVX <= 0) ballVX = -ballVX;
	if(ballY + ballSize + ballVY >= height || ballY + ballVY <= 0) ballVY = -ballVY;
	if(ballY - ballSize <= barY + barHeight) { 
		if(ballX >= barX && ballX + ballSize <= barX + barWidth) {
			ballVY = -ballVY;
		} else {
			ballY = height / 2; 
			ballX = width / 2;
	...
	for(int i = 0; i < blocksX; i++) {
		for(int j = 0; j < blocksY; j++) {
			if(blocks[i][j]) {
				int x = (float) i / blocksX * width + 1;
				int y = height - ((float) j / blocksY * (height / 4) + 6);
				int x2 = x + (width / blocksX);
				int y2 = y + (height / 4 / blocksY);
				DrawFillRect(x, y, x2, y2, 1);

				if(ballY + ballSize >= y && ballY <= y2 && ballX >= x && ballX + ballSize <= x2) {
					blocks[i][j] = 0;
					ballVY = -ballVY;
				}
			}
	...
	DrawHLine(0, height, height, 1);
	DrawFillRect( barX, barY, barX + barWidth, barY + barHeight, 1 );
	DrawFillRect( ballX, ballY, ballX + ballSize, ballY + ballSize, 1);
